...
async function testProgressiveContentAppearance(page) {
  ...
  const criticalContentVisible = await page.waitForSelector('[data-testid="critical-metrics"]', {
    timeout: 5000
  ...
  if (criticalContentVisible) {
    secondaryContentVisible = await page.waitForSelector('[data-testid="secondary-metrics"]', {
      timeout: 10000
    }).then(() => true).catch(() => false);
  }
  ...
  if (secondaryContentVisible) {
    detailedContentVisible = await page.waitForSelector('[data-testid="detailed-content"]', {
      timeout: 15000
    }).then(() => true).catch(() => false);
  }
...
async function testSkeletonComponents(page) {
  ...
  const skeletonComponents = await page.$$('[data-testid="skeleton"]');
  ...
  const initialLayout = await page.evaluate(() => {
    const criticalSection = document.querySelector('[data-testid="critical-metrics"]');
    if (criticalSection) {
...
async function testFadeInAnimations(page) {
  ...
  const fadeInAnimation = await page.evaluate(() => {
    const sections = document.querySelectorAll('[data-testid="section-wrapper"]');
...
async function testErrorHandling(page) {
  ...
  const errorComponents = await page.$$('[data-testid="error-boundary"]');
...
async function testRetryFunctionality(page) {
  ...
  const refreshButton = await page.$('[data-testid="refresh-all-button"]');
...
async function testEdgeCases(page) {
  ...
  const navigationHandled = await page.evaluate(() => {
    return document.querySelector('[data-testid="critical-metrics"]') !== null;
  });
